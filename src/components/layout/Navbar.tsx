...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 h-[60px] bg-white/40 backdrop-blur-sm">
      <div className="flex items-center justify-between h-full w-[1593px] max-w-full mx-auto px-4">
        {/* Logo 区域 - 精确尺寸 w-[309px] */}
        <Link to="/" className="flex items-center justify-between w-[309px]">
          <img src={logo} alt="百得思维" className="w-[87px] h-[36px] object-contain" />
          <div className="w-[1px] h-[24px] bg-black/10" />
          <img src={logoText} alt="思得创新 返回定量可预期" className="w-[179px] h-[16px] object-contain" />
        </Link>

        {/* Desktop Navigation - 精确尺寸 w-[817px] */}
        <div className="hidden lg:flex items-center justify-between w-[817px]">
          {navItems.map((item) => (
            <div
              key={item.label}
            ...
            >
              {item.children ? (
                <>
                  <button className="font-['PingFang_SC'] text-[21px] font-medium text-[#303133] hover:text-[#3366ff] transition-colors leading-normal">
                    {item.label}
                  </button>
                ...
                <Link
                  to={item.path || '/'}
                  className="font-['PingFang_SC'] text-[21px] font-medium text-[#303133] hover:text-[#3366ff] transition-colors leading-normal"
                >
                  {item.label}
                </Link>
              )}
